...
class Player(pygame.sprite.Sprite):
	...
	def load_player(self, data, all_weapons):
		...
		# ~ for name, weapon in self.weapons.items():
			...
			# ~ weapon.lvl   = data['weapons'][name]['lvl']
			# ~ weapon.str   = data['weapons'][name]['str_']
			# ~ weapon.ammo  = data['weapons'][name]['ammo']
			# ~ weapon.tps   = data['weapons'][name]['tps']
			# ~ weapon.dofs  = data['weapons'][name]['dofs']
			# ~ weapon.speed = data['weapons'][name]['speed']
			# ~ weapon.accuracy = data['weapons'][name]['acc']
			# ~ weapon.sp    = data['weapons'][name]['sp']
			# ~ else:
				# ~ pos = 2
				# ~ if name == 'Plasma'
				# ~ self.update_weapons(self.all_weapons[pos])
	...
	def move(self, config, objs, enemies):
		...
		if not enemies and config.speed_up:
			if config.dir_l: self.x -= self.speed * config.speed_delta * 3
			if config.dir_r: self.x += self.speed * config.speed_delta * 3
			if config.dir_u: self.y -= self.speed * config.speed_delta * 3
			if config.dir_d: self.y += self.speed * config.speed_delta * 3
		else:
		...
		for w in objs:
			...
			if result:
				...
				if not enemies and config.speed_up:
					if   135 < angle <= 225: self.x += self.speed*2 * config.speed_delta * 3
					if    45 < angle <= 135: self.y += self.speed*2 * config.speed_delta * 3
					if   -45 < angle <=  45: self.x -= self.speed*2 * config.speed_delta * 3
					if  angle <= -45 or angle > 225: self.y -= self.speed*2 * config.speed_delta * 3
				else:
